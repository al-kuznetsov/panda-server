...
public class PersistentDataProviderService {
  ...
  private AnimalIndicators createAnimalIndicators() {
    return AnimalIndicators.builder()
        ...
        .mildDamageCount(ThreadLocalRandom.current().nextInt(0, 5))
        .mobilityLossLevel(MobilityLossLevel.getRandomMobilityLossLevel())
        .appetiteLevel(AppetiteLevel.getRandomAppetiteLevel())
        .hasSymptoms(ThreadLocalRandom.current().nextBoolean())
        .isPregnant(false)
        .aggressionLevel(AggressionLevel.getRandomAggressionLevel())
        .build();
  }
  ...
  public List<Animal> newListOfAnimals(boolean persist) {
    ...
    animals.add(
        Animal.builder()
            ...
            .birthDate(null)
            .description(
                "Вот такая трехцветняа красотка ОЧЕНЬ нуждается в заботливом хозяине. Пушистая, послушная, ласковая")
            .fullBio(ResourceConst.EMPTY_STRING_PLACEHOLDER)
            .imageUrl(ResourceConst.DEFAULT_ANIMAL_PLACEHOLDER_IMG_URL)
            .active(true)
            .type(catType)
    ...
    animals.add(
        Animal.builder()
            ...
            .description(
                "ПОСТ ПО ПРОСЬБЕ.Найден щенок подросток , задние лапы волочит,наверное сбила машина")
            .fullBio(
                "Карачи! ТК Муравейник!Помогите с транспортировкой и лечением щенка. Звонить по телефону +799999999999")
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                "Вчера уже ночью у нас рядом с домой (Транспортная ) в мусорный бак (нового типа) были выброшены в пакете 4 совсем маленьких котёнка (слепые ещё). Соседи достали из бака. Сейчас находятся у соседки. Номер для связи +79277777777 Катя. КОТЯТА БЕЗ КОШКИ. НУЖНА КОШКА ДЛЯ ВЫКОРМКИ И ВЫЖИВАНИЯ КОТЯТ. ПРОСИМ ОТКЛИКНУТЬСЯ")
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                "Найден полупородистый пес в районе 18 мкр-на. Старые или новые хозяева отзовитесь. Пес вполне себе воспитанный")
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                "Около заброшенных вагончиков живёт сиамская кошечка, появилась недавно, ручная, чистая, упитанная. Может кто-то захочет приютить")
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                "Кот в плохом состоянии. Вероятно он болен! Другие коты его обижают! Помогите котику!")
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                "Заметили данного пса в районе Тепличной улицы п.Кушкуль. Пес очень дружелюбный, но к сожалению все уши увешаны клещами. Пес нуждается в лечении и обработке от паразитов. Неравнодушных просьба помочь.")
            .imageUrl(ResourceConst.DEFAULT_ANIMAL_PLACEHOLDER_IMG_URL)
            .active(true)
            .type(dogType)
            .status(strayStatus)
        ...
        Animal.builder()
            ...
            .description(
                "Найден мальчик такса в ошейнике. Напуган, дрожит"
                    + "На ухе желтая бирка, стерилизована и привита.")
            .fullBio(
                "Срочно ребята !!! Люди нашли таксу мальчик улица Чкалова в ошейнике завели в подъезд "
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                "Найден щенок. Он истощён. Держать дома его не смогут, люди снимают квартиру хозяйка против."
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                "Остановка 19 микрорайон, на дороге мать сбила машина, щенок сидит возле дороги, не уходит.")
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                " Кошка очень добрая, ласковая, сама просит помощи будто. Скажите, вы могли бы помочь?"
    ...
    animals.add(
        Animal.builder()
            ...
            .fullBio(
                " У супермаркета Пятерочки  ходит жалкого вида худой одноглазый серый кот. Голодный, ел жадно с мурлыканьем."
